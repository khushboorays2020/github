...
public class UserModelHibImpl implements UserModelInt {
	...
	public UserDTO findByLogin(String login) throws ApplicationException {
		...
		try {
			...
			/* if (list.size() > 0) {dto = (UserDTO) list.get(0);} ---OR---
			 * while (it.hasNext()) { UserDTO object = (UserDTO) it.next();} if (list.size()
			 * <= 0) {return dto;} dto = (UserDTO) list.get(0);
			 */
	...
	/*	By Using Criteria
		...
		try {
			...
			while (true) {
				if (!it.hasNext()) {
					if (list.size() > 0) {dto = (UserDTO) list.get(0);} 
					else { dto = null; } break;
				} UserDTO var8 = (UserDTO) it.next(); }
		} catch (HibernateException var12) {
			throw new ApplicationExcepton("exception in user authenticaton" + var12.getMessage());
